...
DATE_WITH_DOTS_PATTERN = re.compile(r"\b([0-3]?\d)[./-]([01]?\d)(?:[./-](\d{2,4}))?\b")
...
BOOKING_REQUEST_TEXT = (
    ...
    "Напишите дату и время в формате ДД.ММ ЧЧ:ММ, например 18.03 14:30."
)
...
class MessageProcessor:
    ...
    def process(self, dto: IncomingMessageDTO) -> ProcessResult:
        ...
        company_knowledge = load_company_knowledge(
            directory=self.settings.company_knowledge_dir,
            max_files=self.settings.company_knowledge_max_files,
            max_chars=self.settings.company_knowledge_max_chars,
        )
    ...
    def _resolve_selected_slot(
        ...
        if parsed_slot is None:
            return None
        if not self._is_future_consultation_time(parsed_slot, now_utc=now_utc):
            return None
        return parsed_slot
    ...
    def _resolve_booking_slot(
        ...
        *,
        ...
        by_ai = self._resolve_selected_slot(ai_selected_slot, now_utc)
    ...
    def _resolve_slot_from_user_text(self, *, user_text: str, now_utc: datetime) -> datetime | None:
        ...
        date_match = DATE_WITH_DOTS_PATTERN.search(text)
        time_match = TIME_WITH_MINUTES_PATTERN.search(text)
        ...
        try:
            candidate_slot = datetime(year=year, month=month, day=day, hour=hour, minute=minute, tzinfo=MOSCOW_TZ)
        except ValueError:
            return None

        if not self._is_future_consultation_time(candidate_slot, now_utc=now_utc):
            return None

        return candidate_slot

    def _is_future_consultation_time(self, slot_msk: datetime, *, now_utc: datetime) -> bool:
        now_msk = now_utc.astimezone(MOSCOW_TZ)
